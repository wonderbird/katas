...
var _ = Describe("DecodeBits", func() {
	DescribeTable("Decode single codewords",
		...
		CreateEntryWithAutoGeneratedDescription("01110", "."),
	)
	...
	DescribeTable("Decode two codewords",
		...
		CreateEntryWithAutoGeneratedDescription("1110111", "--"),
	)

	//	FDescribeTable("Regressions (this table is used for debugging; entries are moved to the corresponding table after finishing the bugfix",
	//		WhenInputThenDecodeBitsReturnsExpected,
	//	)
	...
	It("Pause of 3 times time unit between characters is converted to single space", func() {
		Expect(DecodeBits("1100000011001111")).To(Equal(". .-"))
	})

	It("Pause of 7 times time unit between characters is converted to 3 spaces", func() {
		Expect(DecodeBits("110000000000000011001111")).To(Equal(".   .-"))
	})
})
